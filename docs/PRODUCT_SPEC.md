...
自然完成日期的日历推进速度始终按选定完整窗口计算：7 天窗口除以 7、14 天窗口除以 14、30 天窗口除以 30；不得改用实际阅读日数量作为分母。正常结束但起止页相同的 Session 仍计入 Session 数、阅读日、总阅读时长和整体速度分母，其推进页数为 0。若选定窗口总推进页数为 0，则整体速度不可用于预测，不显示剩余阅读时间或自然完成日期。

完成日期必须先满足全部硬门槛：至少 5 个合格 Session、3 个阅读日、数据跨度不少于 7 个自然日、最近 14 天至少一次合格阅读、剩余页数大于 0、日历推进速度大于 0、Learning Item 为 `IN_PROGRESS`，且近期速度未达到高波动。首版使用实施计划冻结的 leave-one-out `robustCV`，`robustCV > 0.75` 视为高波动；计算过程不得向业务结果泄漏 NaN 或 Infinity。只有硬门槛全部通过后才评估高 / 中 / 低可信度，低可信度不显示日期。高 / 中可信度的完成范围分别使用中心日期的 10% / 20%，最早不早于明天。

如果预测可信度太低：

**不显示预计完成日期，只显示预计剩余阅读时间。**

可信度只显示高 / 中 / 低，不显示百分比。波动、高低可信度与日期范围规则已按上述 Module 2D 口径冻结。
...
- 7 天 vs 前 7 天的长期趋势扩展（基础事实摘要已在 Phase 2）
- Session 历史的全局聚合（单书基础历史已在 Phase 2）
- 阅读速度异常
- 状态相关性
