...
完成日期先通过硬门槛：至少 5 个合格 Session、3 个阅读日、跨度不少于 7 个自然日、最近 14 天有正常阅读、剩余页数和日历推进速度均大于 0、Learning Item 为 `IN_PROGRESS`，且 `robustCV <= 0.75`。这里的 `robustCV` 特指 leave-one-out 最小 CV：依次去掉一个 Session 速度样本，分别计算剩余样本的变异系数，再取其中最小值；它不是传统全样本 CV，也不是 median/MAD。该定义用于避免单次异常值直接否决预测，未来不得在不更新产品决策与测试的情况下替换其含义。7/14/30 天日历推进速度分别固定除以完整 7/14/30 个本地自然日；没有阅读的日期也是现实节奏的一部分。硬门槛通过后才评估高/中/低可信度，低可信度不显示日期。高/中可信度分别使用中心日期的 10%/20% 形成范围。零推进的正常 Session 保留在历史，并计入 Session 数、阅读日、总时长和整体速度分母；窗口总推进为 0 时不产生任何预测。
